...
public class InterviewService {
    ...
    public void ajouter(Interview interview) throws SQLException {
        String sql = "INSERT INTO interview (id_candidat, date, type, job_offer, interviewer, location_link) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        ps.setString(6, interview.getLocationLink());
        ps.executeUpdate();
    }
    ...
    // ------------------- UPDATE -------------------
    public void update(int id, int candidateId, LocalDateTime dateTime, String type, String jobOffer, String interviewer, String locationLink) throws SQLException {
        String sql = "UPDATE interview SET id_candidat=?, date=?, type=?, job_offer=?, interviewer=?, location_link=? WHERE id=?";
        PreparedStatement ps = conn.prepareStatement(sql);
