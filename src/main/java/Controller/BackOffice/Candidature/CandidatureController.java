        ...
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                ...
                try {
                    Candidat candidate = new CandidatureService().getCandidatById(c.getCandidateId());
                    String jobTitle = new CandidatureService().getJobTitleById(c.getJobOfferId());
                    double matchScore = Service.MatchingService.getMatchScore(candidate,
                            new Service.JobOfferService().getJobOfferById(c.getJobOfferId()));

                    String body = "Hello " + candidate.getName() + ",\n\n" +
                            "Congratulations! Your application for the \"" + jobTitle + "\" position has been accepted.\n" +
                            "Your match score for this job was: " + matchScore + "%.\n\n" +
                            "Best regards,\nBlindHire Team";

                    EmailService emailService = new EmailService();
                    emailService.sendEmail(
                            "naymahadtae@gmail.com",
            ...
            confirm.showAndWait().ifPresent(response -> {
                if (response == ButtonType.OK) {
                    ...
                    try {
                        ...
                        String body = "Hello " + candidate.getName() + ",\n\n" +
                                "We regret to inform you that your application for the \"" + jobTitle + "\" position has been rejected.\n" +
                                "Reason: " + reason + "\n" +
                                "Your match score for this job was: " + matchScore + "%.\n\n" +
                                "Best regards,\nBlindHire Team";
