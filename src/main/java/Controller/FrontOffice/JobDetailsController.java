...
public class JobDetailsController {
    ...
    private void calculateMatch() {
        ...
        var candidate = candidatService.getCurrentCandidate();

        if (candidate == null || job == null) {
            matchPercentageLabel.setText("0% Match");
            matchProgressBar.setProgress(0);
            return;
        }

        double percentage = MatchingService.getMatchScore(candidate, job);

        matchPercentageLabel.setText(String.format("%.0f%% Match", percentage));
        matchProgressBar.setProgress(percentage / 100.0);
    }
